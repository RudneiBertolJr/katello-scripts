...
if(!@repolist) {
  ...
  @repolist = `pulp-admin -u admin -p $password repo list -s | awk '{print \$1}' `;
}
else {
...
foreach my $repo (sort(@repolist)) {
  ...
  my @repoerrata = `pulp-admin -u admin -p $password rpm repo content errata --repo-id=$repo --fields=id | grep Id: | awk '{print \$2}' `;
  chomp @repoerrata;
  ...
  my @allpkg = `pulp-admin -u admin -p $password rpm repo content rpm --repo-id=$repo --fields=filename | grep Filename: | awk '{print \$2}' `;
  chomp @allpkg;
  ...
  if (not(defined($existing{$advid}))) {
    ...
    if (@packages >= 1) {
      ...
      foreach my $package (@packages) {
        ...
        @pkgdetails = `pulp-admin -u admin -p $password rpm repo content rpm --repo-id=$name2channel{$package} --match="filename=$filename" --fields=name,version,release,epoch,arch,checksum,checksumtype | awk '{print \$2}'`;
        chomp @pkgdetails;
